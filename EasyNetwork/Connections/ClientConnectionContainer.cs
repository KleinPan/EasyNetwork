...
{
    /// <summary>
    /// Provides convenient methods to reduce the number of code lines which are needed to manage all the connections. By default one tcp and one udp connection will be created automatically.
    /// <para> 提供方便的方法来减少管理所有连接所需的代码行数。默认情况下，将自动创建一个tcp和一个udp连接。 </para>
    /// </summary>
    public class ClientConnectionContainer : ConnectionContainer, IPacketHandler, IDisposable
    {
        #region Variables

        /// <summary> The reconnect timer. Invoked if we lose the connection. </summary>
        private Timer reconnectTimer = new Timer();

        /// <summary> The <see cref="Network.TcpConnection"/> for this <see cref="ClientConnectionContainer"/>. </summary>
        private TcpConnection tcpConnection;

        /// <summary> The <see cref="Network.UdpConnection"/> for this <see cref="ClientConnectionContainer"/>. </summary>
        ...
        /// <summary> Initializes a new instance of the <see cref="ClientConnectionContainer"/> class. </summary>
        ...
        /// <summary> Whether to automatically attempt to reconnect to the remote endpoint once the connection is lost. </summary>
        public bool AutoReconnect { get; set; } = true;

        /// <summary> The interval in milliseconds between reconnect attempts. </summary>
        ...
        /// <summary> The current <see cref="Network.TcpConnection"/> for this instance. </summary>
        public TcpConnection TcpConnection { get { return tcpConnection; } }

        /// <summary> The current <see cref="Network.UdpConnection"/> for this instance. </summary>
        public UdpConnection UdpConnection { get { return udpConnection; } }

        /// <summary> Whether the <see cref="TcpConnection"/> is currently alive. </summary>
        ...
        /// <summary> Whether the <see cref="UdpConnection"/> is currently alive. </summary>
        ...
        /// <summary> Whether both the <see cref="TcpConnection"/> and <see cref="UdpConnection"/> are currently alive. </summary>
        public bool IsAlive { get { return IsAlive_TCP && IsAlive_UDP; } }

        /// <summary> If the <see cref="Send(Packet)"/> (or any other sending related method) gets called and the corresponding connection isn't alive, the <see cref="Packet"/> can't be sent to the endpoint. Hence, the <see cref="Packet"/> is undeliverable. In such a chase, this property indicates whether to throw an <see cref="ConnectionNotAliveException"/>. </summary>
        ...
        /// <summary> Signifies that a connection has been made on either the <see cref="TcpConnection"/> or <see cref="UdpConnection"/>. </summary>
        ...
        /// <summary> Signifies that a connection has been lost on either the <see cref="TcpConnection"/> or <see cref="UdpConnection"/>. </summary>
        ...
        /// <summary> Initialises this <see cref="ClientConnectionContainer"/> instance and attempts to connect to the current <see cref="Connection.IPRemoteEndPoint"/>. </summary>
        ...
        /// <summary> Tries to connect to the given endpoint. </summary>
        /// <param name="sender"> The sender of the event. </param>
        /// <param name="e">      Any event arguments. </param>
        private void TryToConnect(object sender, ElapsedEventArgs e) => TryConnect();

        /// <summary> Tries to connect to the current <see cref="Connection.IPRemoteEndPoint"/>. </summary>
        ...
        /// <summary> Closes the <see cref="TcpConnection"/> and <see cref="UdpConnection"/> with the given <see cref="CloseReason"/>, optionally calling the <see cref="Connection.ConnectionClosed"/> event. </summary>
        ...
        /// <summary> Opens the new TCP connection and applies any buffered (i.e. already registered) packet handlers. </summary>
        private async Task OpenNewTCPConnection()
        {
            ...
            // add pre-existing known types and the previous internal packet delegate invoke structure. Hence, the new connection is able to pick up the state of the previous TCP-Connection.
        ...
        /// <summary> Opens the new UDP connection and applies any buffered (i.e. already registered) packet handlers. </summary>
        private async Task OpenNewUDPConnection()
        {
            ...
            // add pre-existing known types and the previous internal packet delegate invoke structure. Hence, the new connection is able to pick up the state of the previous UDP-Connection.
        ...
        /// <summary> Sends a ping over the TCP connection. </summary>
        ...
        /// <summary> Sends the given <see cref="Packet"/> to the network via TCP. The sender will not receive an answer, due to no sender instance being given. </summary>
        /// <param name="packet"> The packet to send. </param>
        public void Send(Packet packet) => SendSlow(packet);

        /// <summary> Sends the given <see cref="Packet"/> to the network, via the given <see cref="ConnectionType"/>. </summary>
        /// <param name="packet"> The packet to send. </param>
        ...
        /// <summary> Sends the given <see cref="Packet"/> over the network via TCP and awaits a <see cref="ResponsePacket"/> on the given <see cref="object"/> instance. </summary>
        /// <param name="packet">   The packet to send. </param>
        ...
        /// <summary> Asynchronously sends the given <see cref="Packet"/> over the network via TCP and awaits a <see cref="ResponsePacket"/> of the given type. </summary>
        /// <typeparam name="T"> The type of <see cref="ResponsePacket"/> to await. </typeparam>
        /// <param name="packet"> The packet to send. </param>
        /// <returns> A <see cref="Task"/> representing the asynchronous operation, with the promise of the received <see cref="ResponsePacket"/> on completion. </returns>
        public async Task<T> SendAsync<T>(Packet packet) where T : ResponsePacket => await SendAsync<T>(packet, ConnectionType.TCP);

        /// <summary> Sends the given <see cref="Packet"/> over the network via the given <see cref="ConnectionType"/> and awaits a <see cref="ResponsePacket"/> on the given <see cref="object"/> instance. </summary>
        /// <param name="packet">   The packet to send. </param>
        /// <param name="instance"> The sender instance to receive a response. </param>
        /// <param name="type">     The connection type to use. </param>
        /// <exception cref="ArgumentException"> Thrown when the given <see cref="ConnectionType"/> value is an invalid cast. </exception>
        ...
        /// <summary> Asynchronously sends the given <see cref="Packet"/> over the network via the given <see cref="ConnectionType"/> and awaits a <see cref="ResponsePacket"/> of the given type. </summary>
        /// <typeparam name="T"> The type of <see cref="ResponsePacket"/> to await. </typeparam>
        /// <param name="packet">         The packet to send. </param>
        ...
        /// <summary> Sends the given <see cref="Packet"/> to the network via UDP. The sender will not receive an answer, due to no sender instance being given. </summary>
        /// <param name="packet"> The packet to send. </param>
        ...
        /// <summary> Sends the given <see cref="Packet"/> over the network via UDP and awaits a <see cref="ResponsePacket"/> on the given <see cref="object"/> instance. </summary>
        /// <param name="packet">   The packet to send. </param>
        /// <param name="instance"> The sender instance to receive a response. </param>
        ...
        /// <summary> Asynchronously sends the given <see cref="Packet"/> over the network via UDP and awaits a <see cref="ResponsePacket"/> of the given type. </summary>
        /// <typeparam name="T"> The type of <see cref="ResponsePacket"/> to await. </typeparam>
        /// <param name="packet"> The packet to send. </param>
        /// <returns> A <see cref="Task"/> representing the asynchronous operation, with the promise of the received <see cref="ResponsePacket"/> on completion. </returns>
        ...
        /// <summary> Reconnects both the <see cref="TcpConnection"/> and <see cref="UdpConnection"/>. </summary>
        ...
        /// <summary> Creates a new <see cref="Network.TcpConnection"/>. </summary>
        ...
        /// <summary> Creates a new <see cref="Network.UdpConnection"/>. </summary>
