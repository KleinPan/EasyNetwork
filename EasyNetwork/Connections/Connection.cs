...
using EasyNetwork.Utilities;

namespace EasyNetwork.Connections
{
    /// <summary>
    /// Provides the basic methods that all <see cref="Connection"/> inheritors must implement. It ensures connectivity and keeps tracks of statistics such as latency. Multi-threaded with 3 separate threads per connection. After calling the <see cref="Close"/> method, every queued <see cref="Packet"/> will be sent before the <see cref="Connection"/> is fully closed.
    /// <para> </para>
    /// 提供所有 <see cref=" Connection"/> 继承者都必须实现的基本方法。它确保连接性并跟踪诸如延迟之类的统计信息。多线程，每个连接具有3个单独的线程。调用 <see cref="Close"/> 方法后，将在完全关闭 <see cref="Connection"/> 之前发送每个排队的 <see cref="Packet"/>。
    /// </summary>
    /// <remarks>
    ...
    public abstract partial class Connection : IPacketHandler
    {
        ...
        /// <summary> The time interval in milliseconds between ping packets. </summary>
        protected const int PING_INTERVALL = 10000;

        /// <summary> A fixed hashcode that persists with the <see cref="Connection"/> instance for its entire lifetime. </summary>
        private readonly int hashCode;

        // TODO Remove all occurrences of backing fields for events in favor of new, cleaner 'event?.Invoke(args)' syntax

        /// <summary> A handler which will be invoked if this connection is dead. </summary>
        ...
        /// <summary> A handler which will be invoked if a new connection is established. </summary>
        private event Action<TcpConnection, UdpConnection> connectionEstablished;

        /// <summary> A token source to singal all internal threads to terminate. </summary>
        ...
        /// <summary> Whether this instance should send out a keep alive packet at specific intervals, to ensure there is an alive remote connection. If set to [false] <see cref="RTT"/> and <see cref="Ping"/> wont be enabled/refreshed. </summary>
        private bool keepAlive;

        /// <summary> Stopwatch to keep track of when to send out a new <see cref="PingRequest"/>. </summary>
        private readonly Stopwatch nextPingStopWatch = new Stopwatch();

        /// <summary>
        /// Stopwatch measuring elapsed time since the last <see cref="PingRequest"/> was sent to measure the RTT and ping to the remote <see cref="Connection"/>.
        /// <para> 提供了一组方法和属性，可用于准确测量经过的时间。 </para>
        /// </summary>
        ...
        /// <summary> An event set whenever a packet is received from the network. Used to save CPU time when waiting for a packet to be received. </summary>
        private readonly AutoResetEvent packetAvailableEvent = new AutoResetEvent(false);

        /// <summary> An event set whenever a packet is available to be sent to the network. Used to save CPU time when waiting to send a packet. </summary>
        ...
        /// <summary> Reads packets from the network and places them into the <see cref="receivedPackets"/> and <see cref="receivedUnknownPacketHandlerPackets"/> queues. </summary>
        private Thread readStreamThread;

        /// <summary> Handles received packets by invoked their respective <see cref="PacketReceivedHandler{P}"/>. </summary>
        private Thread invokePacketThread;

        /// <summary> Sends pending packets to the network from the <see cref="sendPackets"/> queue. </summary>
        ...
        /// <summary> The packet converter used to serialise and deserialise outgoing and incoming packets. </summary>
        private IPacketConverter packetConverter = new PacketConverter();

        /// <summary> Holds all the <see cref="UdpConnection"/> s that are currently pending connection to this <see cref="Connection"/>. </summary>
        private readonly ConcurrentQueue<UdpConnection> pendingUDPConnections = new ConcurrentQueue<UdpConnection>();

        /// <summary> Holds all received <see cref="Packet"/> s whose <see cref="Packet.ID"/> is not known. </summary>
        private readonly ConcurrentQueue<Tuple<Packet, object>> pendingUnknownPackets = new ConcurrentQueue<Tuple<Packet, object>>();

        /// <summary> Holds all received <see cref="Packet"/> s with a known <see cref="PacketReceivedHandler{P}"/> that are yet to be handled. </summary>
        private readonly ConcurrentQueue<Packet> receivedPackets = new ConcurrentQueue<Packet>();

        /// <summary> Holds all received <see cref="Packet"/> s without a known <see cref="PacketReceivedHandler{P}"/> that are yet to be handled. </summary>
        private readonly ConcurrentBag<Packet> receivedUnknownPacketHandlerPackets = new ConcurrentBag<Packet>();

        /// <summary> Holds all <see cref="Packet"/> s that are handled and are now ready and waiting to be sent to the network. </summary>
        private readonly ConcurrentQueue<Tuple<Packet, object>> sendPackets = new ConcurrentQueue<Tuple<Packet, object>>();

        /// <summary> Maps a <see cref="Packet"/><see cref="Type"/> to a unique <see cref="ushort"/> ID. </summary>
        private readonly BiDictionary<Type, ushort> typeByte = new BiDictionary<Type, ushort>();

        /// <summary> The value from which new IDs for packet <see cref="Type"/> s will be calculated dynamically. Starts at 100 as the library already has built-in packets. </summary>
        private int currentTypeByteIndex = 100;

        /// <summary> Maps a <see cref="RequestPacket"/><see cref="Type"/> to the <see cref="Type"/> of the <see cref="ResponsePacket"/> that handles it. </summary>
        private readonly Dictionary<Type, Type> requestResponseMap = new Dictionary<Type, Type>();

        /// <summary> Maps <see cref="Packet"/><see cref="Type"/> s to the <see cref="PacketReceivedHandler{P}"/> that should be used for that <see cref="Packet"/>. </summary>
        ...
        /// <summary> Partial initialisation method to be overriden in later class implementations. </summary>
        partial void InitAddons();

        /// <summary> Initialises this <see cref="Connection"/> instance, setting up all required variables. </summary>
        internal void Init()
        {
            //目前只有日志插件
            InitAddons();
...
#if !DEBUG
        /// <summary> The timeout value in milliseconds. If the connection does not receive any packet within the specified timeout, the connection will timeout and shutdown. </summary>
        ...
        /// <summary> The amount of <see cref="Packets"/> that are pending handling that this <see cref="Connection"/> will buffer. If we receive a packet which has no handler, it will be buffered for future handler registrations (via <see cref="RegisterStaticPacketHandler{P}(PacketReceivedHandler{P})"/>, <see cref="RegisterPacketHandler{P}(PacketReceivedHandler{P}, object)"/>, and <see cref="RegisterRawDataHandler"/> or their overloads). This value indicates the maximum amount of <see cref="Packet"/> s that will be buffered before any are dropped. </summary>
        /// <value> The packet buffer. </value>
        public int PacketBuffer { get; set; } = 1000;

        /// <summary> Whether this <see cref="Connection"/> is alive and able to communicate with the <see cref="Connection"/> at the <see cref="IPRemoteEndPoint"/>. </summary>
        ...
        /// <summary> The local <see cref="IPEndPoint"/> for this <see cref="Connection"/> instance. </summary>
        public abstract IPEndPoint IPLocalEndPoint { get; }

        /// <summary> The remote <see cref="IPEndPoint"/> that this <see cref="Connection"/> instance communicates with. </summary>
        public abstract IPEndPoint IPRemoteEndPoint { get; }

        /// <summary> Whether this <see cref="Connection"/> can operate in dual IPv4 / IPv6 mode. </summary>
        public abstract bool DualMode { get; set; }

        /// <summary> Whether sending a packet flushes underlying <see cref="NetworkStream"/>. </summary>
        /// <remarks> This value is only used in a <see cref="TcpConnection"/> instance, which uses a <see cref="NetworkStream"/> to send and receive data. A <see cref="UdpConnection"/> is unaffected by this value. </remarks>
        public bool ForceFlush { get; set; } = true;

        /// <summary> Whether this <see cref="Connection"/> is allowed to fragment frames that are too large to send in one go. </summary>
        public abstract bool Fragment { get; set; }

        /// <summary> The hop limit for packets sent by this <see cref="Connection"/>. Comparable to IPv4s TTL (Time To Live). </summary>
        public abstract int HopLimit { get; set; }

        /// <summary> Whether the packet should be sent directly to its destination or allowed to be routed through multiple destinations first. </summary>
        public abstract bool IsRoutingEnabled { get; set; }

        /// <summary> Whether the packet should be send with or without any delay. If disabled, no data will be buffered at all and sent immediately to it's destination. There is no guarantee that the network performance will be increased. </summary>
        public abstract bool NoDelay { get; set; }

        /// <summary> The 'Time To Live' for this <see cref="Connection"/>. </summary>
        public abstract short TTL { get; set; }

        /// <summary> Whether this <see cref="Connection"/> should use a loopback address and bypass hardware. </summary>
        public abstract bool UseLoopback { get; set; }

        #endregion Socket Properties

        /// <summary> Whether this <see cref="Connection"/> should send a keep alive packet to the <see cref="IPRemoteEndPoint"/> at specific intervals, to ensure whether there is a remote <see cref="Connection"/> or not. If set to <c> false </c><see cref="RTT"/> and <see cref="Ping"/> won't be refreshed automatically. </summary>
        ...
        /// <summary> The Round Trip Time for a packet. </summary>
        public virtual long RTT { get; protected set; } = 0;

        /// <summary> The ping to the <see cref="IPRemoteEndPoint"/>. </summary>
        public virtual long Ping { get; protected set; } = 0;

        /// <summary> Gets or sets the performance of this <see cref="Connection"/>. The higher the sleep intervals (the lower the performance), the slower this <see cref="Connection"/> will handle incoming, pending handling, and outgoing <see cref="Packet"/> s. </summary>
        public Performance Performance { get; set; } = Performance.Default;

        /// <summary> The value of <see cref="Performance"/>, but simply cast to an <see cref="int"/>. </summary>
        public int IntPerformance { get { return (int)Performance; } }

        /// <summary> Allows the usage of a custom <see cref="IPacketConverter"/> implementation for serialisation and deserialisation. However, the internal structure of the packet should stay the same: Packet Type : 2 bytes (ushort) Packet Length : 4 bytes (int) Packet Data : xx bytes (actual serialised packet data) </summary>
        /// <remarks> The default <see cref="PacketConverter"/> uses reflection (with type property caching) for serialisation and deserialisation. This allows good performance over the widest range of packets. Should you want to handle only a specific set of packets, a custom <see cref="IPacketConverter"/> can allow more throughput (no slowdowns due to relatively slow reflection). </remarks>
        ...
        /// <summary> Event signifying that a connection was closed between this <see cref="Connection"/> instance and another <see cref="Connection"/>. This event is only visible for the network library itself. It garantuees, that the lib itself is capable of receiving connection state changes. </summary>
        ...
        /// <summary> Event signifying that a connection was closed between this <see cref="Connection"/> instance and another <see cref="Connection"/>. </summary>
        ...
        /// <summary> Event signifying that this <see cref="Connection"/> instance established a new connection with either a <see cref="TcpConnection"/> or <see cref="UdpConnection"/> instance. </summary>
        ...
        /// <summary> Returns the current <see cref="PacketHandlerMap"/> instance, so that the types of packets handled can be read. </summary>
        /// <returns> The current <see cref="PacketHandlerMap"/> instance used by this connection. </returns>
        internal PacketHandlerMap BackupPacketHandler() => packetHandlerMap;

        /// <summary> Invoked whenever the <see cref="packetHandlerMap"/> gets refreshed. </summary>
        ...
        /// <summary> Configures the <see cref="nextPingStopWatch"/> timer. </summary>
        /// <param name="enable"> Whether the <see cref="nextPingStopWatch"/> should be enabled on reconfiguring. </param>
        ...
        /// <summary> Sends a <see cref="PingRequest"/>, if a <see cref="PingResponse"/> is not currently being awaited. </summary>
        ...
        /// <summary> Sends the given <see cref="Packet"/> and asynchronously awaits the <see cref="ResponsePacket"/>. </summary>
        /// <typeparam name="T"> The <see cref="ResponsePacket"/> type to await. </typeparam>
        /// <param name="packet"> The <see cref="Packet"/> to send. </param>
        /// <returns> A <see cref="Task{T}"/> representing the asynchronous operation, with the promise of a <see cref="ResponsePacket"/> of the given type upon completion. </returns>
        public async Task<T> SendAsync<T>(Packet packet) where T : ResponsePacket => await new ChickenReceiver().Send<T>(packet, this);

        /// <summary> Serialises the given <see cref="Packet"/> using the current <see cref="PacketConverter"/> and queues it to be sent to the network. No response is possible as a sender instance is not provided. This method is suitable for static classes and basic packets with no inheritance. </summary>
        /// <param name="packet"> The <see cref="Packet"/> to be sent across the network. </param>
        public void Send(Packet packet) => Send(packet, null);

        /// <summary> Serialises the given <see cref="Packet"/> using the current <see cref="PacketConverter"/> and queues it to be sent to the network. </summary>
        /// <param name="packet">   The <see cref="Packet"/> to be sent across the network. </param>
        ...
        /// <summary> Reads bytes from the network. </summary>
        ...
        /// <summary> Writes bytes to the network. </summary>
        ...
        /// <summary> Invokes the relevant <see cref="PacketReceivedHandler{P}"/> for each packet in the <see cref="receivedPackets"/> queue. </summary>
        ...
        /// <summary> Writes any packets queued up in the <see cref="sendPackets"/> queue to the network. </summary>
        ...
        /// <summary> Handles all default <see cref="Packet"/> s that are in the library. </summary>
        ...
        /// <summary> Invoked for any received <see cref="Packet"/> s that don't have a registered <see cref="PacketReceivedHandler{P}"/>. </summary>
        ...
        /// <summary> Handles the unknown packet. </summary>
        ...
        /// <summary> Handles a <see cref="Connection"/> closure, with the given <see cref="CloseReason"/>. </summary>
        ...
        /// <summary> Closes the socket and frees all associated resources. </summary>
        protected abstract void CloseSocket();

        /// <summary> Handles a case where the remote <see cref="Connection"/> caused a closure for the given <see cref="CloseReason"/>. </summary>
        /// <param name="closeReason"> The reason for the <see cref="Connection"/> closing. </param>
        ...
        /// <summary> Unlocks the connection and allows for data to be sent and received. </summary>
        ...
        /// <summary> Gets a free port that is currently not in use and returns it. </summary>
        ...
        /// <summary> Creates and returns a new <see cref="UdpConnection"/>, with the given local endpoint, remote endpoint, and write lock state. </summary>
        ...
        /// <summary> Returns The unique hashcode of this <see cref="Connection"/> instance. </summary>
